...
- 默认情况下 java.system.class.loader 为 null，那么系统（应用）类加载器是 AppClassLoader

  ```java
  org.yashon.jvm.classloader.ClassLoadingTest17
  ```
...
每个类都会使用自己的类加载器（即加载自身的类加载器）来加载其他类（指的是所依赖的类），如果 ClassX 引用了 ClassY，那么 ClassX 的类加载器就会去加载 ClassY （前提是 ClassY 尚未被加载）



## 线程上下文类加载器(Context Classloader)

Thread 类中的 getContextClassLoader() 与 setContextClassLoader(ClassLoader cl) 分别用来获取和设置线程上下文类加载器

如果没有通过 setContextClassLoader(ClassLoader cl) 进行设置的话，线程将继承父线程的上下文类加载器。Java 应用运行时的初始线程的上下文类加载器是系统类加载器。在线程中运行的代码可以通过该类加载器加载类与资源。
...
- 问题引出
  ...
  > 比如我们在开发中使用的数据库，Java 只提供了接口标准，具体实现由各个厂商实现，这时候就出现了一个问题：java.sql 包下类是由扩展类加载的，而各个厂商的实现打包成 jar 包放在 classpath 下，是由系统（应用）类加载器加载的，**<font color='red'>在双亲委派模式下，父加载器加载的类是无法使用子加载器加载的类；但子加载器加载的类是可以使用父加载器加载的类。</font>** 那么就会出现一个问题：父加载器中的类要调用子加载器加载的类该怎么办？？？

- 解决方案

  > 父 classLoader 可以使用当前线程 Thread.currentThread().getContextClassLoader() 所指定的 classLoader 加载的类。这就改变了父 ClassLoader 不能使用子 ClassLoader 或是其他父子关系的 ClassLoader 加载的类的情况，即改变了双亲委派模型。

- 总结

  > **线程上下文类加载器就是当前线程的 Current ClassLoader**
  >
  > 在双亲委派模型下，类加载是由下至上的，即下层的类加载器会委托上层进行加载。但是对 SPI(Service Provider Interface) 来说，有些接口是 Java 核心库所提供的，而 Java  核心库是由启动类加载器加载的，而这些接口的实现却来自于不同的 jar 包（厂商提供），Java 的启动类加载器是不会加载其他来源的 jar 包，这样传统的双亲委派模型就无法满足 SPI 的要求。而通过给当前线程设置上下文类加载器，就可以由设置的上下文类加载器来实现对于接口实现类的加载。
...
- 线程上下文类加载器的一般使用模式：获取  -----> 使用 -----> 还原
  ...
  try{
    ...
    Thead.currentThread().setContextClassLoader(classLoader);
  }
  ```
...
- serviceLoader 就是解决上面问题的关键代码，下面以一段代码演示
  ...
  public class ClassLoadingTest20 {
      ...
      public static void main(String[] args) {
          ...
          while(iterator.hasNext()) {
              ...
              System.out.println("当前的 Driver: " + element.getClass() 
                                 + ", 加载出来的 Driver 的类加载器: " + element.getClass().getClassLoader());
          }
  ...
  这个类在启动的时候会使用系统（应用）类加载器加载，在 main 方法中主动使用到了 ServiceLoader，那么就会初始化这个类，初始化之前必须加载，那么 ServiceLoader 是通过根类加载器加载的，但是 Driver 的实现类是 mysql 厂商提供的，根类加载器无法加载 mysql 定义的 Driver 类，这时候在 ServiceLoader 中就会拿到当前线程的上下文类加载器（**就是系统（应用）类加载器**）去加载 Driver 的实现类
